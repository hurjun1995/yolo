...
export function signUp(email, password) {
  ...
  return sendRequestAsync(httpMethod.POST, '/user', { email, password }).then(res => res.data)
}

export function logIn(email, password) {
  // TODO: decide on what to return from response object
  return sendRequestAsync(httpMethod.POST, '/user/login', { email, password }).then(res => res.data)
}
